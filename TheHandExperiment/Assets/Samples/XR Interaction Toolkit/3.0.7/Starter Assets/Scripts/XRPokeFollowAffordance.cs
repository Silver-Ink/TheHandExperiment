...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        [SerializeField]
        GameObject cube; // R�f�rence au cube � changer
        ...
        void OnPokeStateDataUpdated(PokeStateData data)
        {
            ...
            else if (m_ReturnToInitialPosition)
            {
                ...
                Debug.Log("<color=green>Poke d�tect� sur l'objet : " + gameObject.name + "</color>");
                isButtonPressed = false;
                ChangeColor script = cube.GetComponent<ChangeColor>();

                if (script != null)
                {
                    // Exemple : Change la couleur en fonction de l'index `color` (ici `color` est d�j� de type ColorType)
